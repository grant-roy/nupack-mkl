...
!
!   Real-to-real 1D transform of type RODFTOO (sine transform) for double
!   precision data
!******************************************************************************/

      PROGRAM R2R_1D_RODFT00_DOUBLE_EX
...
!
      ...
      DO I=1,N
         IN(I)=DSIN(3.1415*DFLOAT(I)/(N+1.))
         EXP_X(I)=IN(I)
      END DO

!
!     Create FFTW plan for 1D real-to-real (sine) transform
!
      PRINT*,' Create FFTW plan for 1D real-to-real (sine) transform'
      CALL DFFTW_PLAN_R2R_1D(FFTW_PLAN,N,IN,OUT,FFTW_RODFT00)
...
!
      PRINT*,' Create FFTW plan for 1D real-to-real (sine) transform'
      CALL DFFTW_PLAN_R2R_1D(FFTW_PLAN,N,OUT,IN,FFTW_RODFT00)
