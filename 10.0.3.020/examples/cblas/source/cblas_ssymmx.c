...
/*       Get input data                                        */
      ...
      if( GetArrayS(in_file, &order, typeA, &ma, &na, a, &lda) != 0 ) {
        printf("\n ERROR of array A reading\n");
        return 0;
      }
      if( GetArrayS(in_file, &order, GENERAL_MATRIX, &m, &n, b, &ldb) != 0 ) {
        printf("\n ERROR of array B reading\n");
        return 0;
      }
      if( GetArrayS(in_file, &order, GENERAL_MATRIX, &m, &n, c, &ldc) != 0 ) {
...
/*       Print input data                                      */
      ...
      PrintArrayS(&order, FULLPRINT, typeA, &ma, &na, a, &lda, "A");
      PrintArrayS(&order, FULLPRINT, GENERAL_MATRIX, &m, &n, b, &ldb, "B");
      PrintArrayS(&order, FULLPRINT, GENERAL_MATRIX, &m, &n, c, &ldc, "C");

/*      Call CBLAS_SSYMM subroutine ( C Interface )            */

      cblas_ssymm(order, side, uplo, m, n, alpha, a, lda,
                  b, ldb, beta, c, ldc);
