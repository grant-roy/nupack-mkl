...
/*       Get input data                                        */
      ...
      if( GetArrayS(in_file, &order, 0, &ma, &na, a, &lda) != 0 ) {
        printf("\n ERROR of array A reading\n");
        return 0;
      }
      if( GetArrayS(in_file, &order, 0, &mb, &nb, b, &ldb) != 0 ) {
        printf("\n ERROR of array B reading\n");
        return 0;
      }
      if (uplo == CblasUpper) typeC = UPPER_MATRIX;
      else                    typeC = LOWER_MATRIX;
      if( GetArrayS(in_file, &order, typeC, &n, &n, c, &ldc) != 0 ) {
...
/*       Print input data                                      */
      ...
      PrintArrayS(&order, FULLPRINT, GENERAL_MATRIX, &ma, &na, a, &lda, "A");
      PrintArrayS(&order, FULLPRINT, GENERAL_MATRIX, &mb, &nb, b, &ldb, "B");
      PrintArrayS(&order, FULLPRINT, typeC, &n, &n, c, &ldc, "C");

/*      Call CBLAS_SSYR2K subroutine ( C Interface )           */

      cblas_ssyr2k(order, uplo, trans, n, k, alpha, a, lda,
                   b, ldb, beta, c, ldc);
