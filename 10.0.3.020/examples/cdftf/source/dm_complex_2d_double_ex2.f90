...
!
       ...
       STATUS = DftiCreateDescriptorDM(MPI_COMM_WORLD,DESC,DFTI_DOUBLE,DFTI_COMPLEX,2,LENGTHS)
...
!          values (for more information, see Distributing Data among Processes).
      ...
      ROOTRANK=0
        ...
        STATUS = MKL_CDFT_SCATTERDATA_D(MPI_COMM_WORLD,ROOTRANK,ELEMENTSIZE,2,LENGTHS,X_IN,NX,START_X,LOCAL)
...
!
      STATUS = MKL_CDFT_GATHERDATA_D(MPI_COMM_WORLD,ROOTRANK,ELEMENTSIZE,2,LENGTHS,X_IN,NX,START_X,WORK)
...
!
      STATUS = MKL_CDFT_SCATTERDATA_D(MPI_COMM_WORLD,ROOTRANK,ELEMENTSIZE,2,LENGTHS,X_IN,NX,START_X,WORK)
        IF(ADVANCED_DATA_PRINT .AND. (MPI_RANK .EQ. 0)) PRINT*,'Scatter=',STATUS
      IF ( STATUS .NE. DFTI_NO_ERROR) THEN
       IF (MPI_RANK .EQ. 0) THEN
       CALL Dfti_Example_Status_Print(STATUS)
...
!
      STATUS = MKL_CDFT_GATHERDATA_D(MPI_COMM_WORLD,ROOTRANK,ELEMENTSIZE,2,LENGTHS,X_IN,NX,START_X,LOCAL)
        IF(ADVANCED_DATA_PRINT .AND. (MPI_RANK .EQ. 0)) PRINT*,'GATHER=',STATUS
      IF ( STATUS .NE. DFTI_NO_ERROR) THEN
       IF (MPI_RANK .EQ. 0) THEN
       CALL Dfti_Example_Status_Print(STATUS)
