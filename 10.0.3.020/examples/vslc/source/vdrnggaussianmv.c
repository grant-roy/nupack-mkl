...
static double C1[NDIM*(NDIM+1)/2]={ 16.0, 8.0, 4.0, 13.0, 17.0, 62.0 };
...
main()
{
    ...
    double  TT[NDIM][NDIM]; // T*T'. Should be equal to var.-covar. matrix
...
/* Testing Choelsky factorization procedure (optional) */
#if defined(TL)
    for(i=0;i<NDIM;i++) {
        for(j=0;j<NDIM;j++) {
            if(j>i) C[i][j] = 0.0;
    ...
    *****************************************************/
    errcode = vdRngGaussianMV( VSL_METHOD_DGAUSSIANMV_BOXMULLER2, stream, N, (double *)r, NDIM, VSL_MATRIX_STORAGE_FULL, a, T );
    CheckVslError( errcode );
...
/* Testing Choelsky factorization procedure (optional) */
    ...
    *****************************************************/
    ...
    /*****************************************************
     ...
     from multivariate normal distribution
    *****************************************************/
    errcode = vdRngGaussianMV( VSL_METHOD_DGAUSSIANMV_BOXMULLER2, stream, N, (double *)r, NDIM, VSL_MATRIX_STORAGE_PACKED, a, T );
    CheckVslError( errcode );
