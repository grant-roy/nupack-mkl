...
int main(void)
{
	...
	float *spar, *f, *u, *bd_ax, *bd_bx, *bd_ay, *bd_by, *bd_az, *bd_bz;
	float q;
	...
	spar=(float*)malloc((13*(nx+ny)/2+9)*sizeof(float));
	f=(float*)malloc((nx+1)*(ny+1)*(nz+1)*sizeof(float));
	u=(float*)malloc((nx+1)*(ny+1)*(nz+1)*sizeof(float));
	...
	*******************************************************************************/
	...
	for(iz=0;iz<=nz;iz++)
	{
		for(iy=0;iy<=ny;iy++)
		{
			for(ix=0;ix<=nx;ix++)
			{
				...
				f[ix+iy*(nx+1)+iz*(nx+1)*(ny+1)]=(12.0E0*pi*pi)*u[ix+iy*(nx+1)+iz*(nx+1)*(ny+1)];
	...
	s_commit_Helmholtz_3D(f, bd_ax, bd_bx, bd_ay, bd_by, bd_az, bd_bz, &xhandle, &yhandle, ipar, spar, &stat);
	if (stat!=0) goto FAILURE;
	...
	s_Helmholtz_3D(f, bd_ax, bd_bx, bd_ay, bd_by, bd_az, bd_bz, &xhandle, &yhandle, ipar, spar, &stat);
	if (stat!=0) goto FAILURE;
	...
	printf("\n real 3D Poisson example has successfully PASSED\n");
...
FAILURE: printf("\nsingle 3D Poisson example FAILED to compute the solution...\n");
