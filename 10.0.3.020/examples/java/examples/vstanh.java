...
public final class vstanh {
     ...
     */
    private final static String test_name = "vstanh";
    ...
    public static boolean test()
    {
        ...
        final float __STANH_BEG = -10.0f;
        final float __STANH_END = 10.0f;
        ...
        for(i=0;i<vec_len;i++) {
                fA[i]=(float)(__STANH_BEG+((__STANH_END-__STANH_BEG)*i)/vec_len);
                fB1[i]=0.0f;
                //fB2[i]=(float)Math.tanh(fA[i]);   // Since 1.5
                fB2[i]=(float)(Math.exp(fA[i])-Math.exp(-fA[i]))/(float)(Math.exp(fA[i])+Math.exp(-fA[i]));
        }

        VML.vsTanh(vec_len,fA,fB1);

        System.out.println("vsTanh test/example program\n");
        System.out.println("           Argument                     vsTanh                     Tanhf");
        System.out.println("===============================================================================");
        for(i=0;i<vec_len;i++) {
