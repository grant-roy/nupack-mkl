 ...
 * 1-dimensional case, inplace transform.
 *
...
public final class dfti_z1i {
     ...
     */
    private final static String test_name = "dfti_z1i";
    ...
    public static boolean test() throws DFTI.StatusException
    {
        ...
        //
        double x_inout[] = new double [2 * length];

        //
        // Create new DFTI descriptor and adjust for inplace mode
        //
        ...
        */
        ...
        boolean fault = false;
        for (int i=0; i<length; i++) {
            System.out.print("\t("+x_inout[2*i]+","+x_inout[2*i+1]+")");
            if (Math.abs(x_inout[2*i]-i)+Math.abs(x_inout[2*i+1]) > tolerance)
                fault = true;
        }
        System.out.println();
