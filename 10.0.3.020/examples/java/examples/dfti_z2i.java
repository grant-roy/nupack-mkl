...
public final class dfti_z2i {
     ...
     */
    private final static String test_name = "dfti_z2i";
    ...
    public static boolean test() throws DFTI.StatusException
    {
        ...
        double x_inout[] = new double[2 * array_size];

        //
        // Create new DFTI descriptor, and adjust for inplace transform
        //
        ...
        */
        ...
        for (int i=0; i<lengths[0]; i++) {
            for (int j=0; j<lengths[1]; j++) {
                int k = i*lengths[1] + j; // row-major
                System.out.print("\t("+x_inout[2*k]+","+x_inout[2*k+1]+")");
                if (Math.abs(x_inout[2*k]-k)+Math.abs(x_inout[2*k+1]) > tolerance)
                    fault = true;
            }
            System.out.println();
        }
