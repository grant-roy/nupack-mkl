...
/**
 * Example of using MKL DFTI: double precision real type,
...
public final class dfti_d2 {
     ...
     */
    private final static String test_name = "dfti_d2";
    ...
    public static boolean test() throws DFTI.StatusException
    {
        ...
        double x_normal[]      = new double[ array_size ];
        double x_transformed[] = new double[ array_size ];
        ...
        */
        ...
        for (int i=0; i<lengths[0]; i++) {
            for (int j=0; j<lengths[1]; j++) {
                int k = i*lengths[1] + j; // row-major
                System.out.print("\t" + x_normal[k]);
                if (Math.abs(x_normal[k]-k) > tolerance)
                    fault = true;
            }
            System.out.println();
        }
