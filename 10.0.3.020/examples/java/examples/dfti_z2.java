...
/**
 * Example of using MKL DFTI: double precision complex type,
 * 2-dimensional case, not-inplace transform.
 *
...
public final class dfti_z2 {
     ...
     */
    private final static String test_name = "dfti_z2";
    ...
    public static boolean test() throws DFTI.StatusException
    {
        ...
        // Parameters
        //
        final double tolerance = 1.0e-13; // absolute error
        final int precision = DFTI.DOUBLE;
        ...
        // allow twice space for complex data
        double x_normal[]      = new double[2 * array_size];
        double x_transformed[] = new double[2 * array_size];

        //
        // Create new DFTI descriptor, and adjust for not-inplace transform
        //
        ...
        */
        ...
        //
        // You do not need to change the default strides in this case.
        ...
        //
        long transform_size = lengths[0] * lengths[1];
        ...
        Double backward_scale = (Double) DFTI.GetValue(desc_handle,DFTI.BACKWARD_SCALE);
        ...
        for (int i=0; i<lengths[0]; i++) {
            for (int j=0; j<lengths[1]; j++) {
                int k = i*lengths[1] + j; // row-major
                System.out.print("\t("+x_normal[2*k]+","+x_normal[2*k+1]+")");
                if (Math.abs(x_normal[2*k]-k)+Math.abs(x_normal[2*k+1]) > tolerance)
                    fault = true;
            }
            System.out.println();
        }
