...
public final class vdtanh {
     ...
     */
    private final static String test_name = "vdtanh";
    ...
    public static boolean test()
    {
        ...
        for(i=0;i<vec_len;i++) {
                dA[i]=(double)(__DTANH_BEG+((__DTANH_END-__DTANH_BEG)*i)/vec_len);
                dB1[i]=0.0;
                //dB2[i]=Math.tanh(dA[i]);   // Since 1.5
                dB2[i]=(Math.exp(dA[i])-Math.exp(-dA[i]))/(Math.exp(dA[i])+Math.exp(-dA[i]));
        }

        VML.vdTanh(vec_len,dA,dB1);

        System.out.println("vdTanh test/example program\n");
        System.out.println("           Argument                     vdTanh                     Tanh");
        System.out.println("===============================================================================");
        for(i=0;i<vec_len;i++) {
