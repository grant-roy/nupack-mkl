...
!          program (C-interface)
!
!   Real-to-complex and complex-to-real 2D transform for double precision data
...
int main()  /* REAL_2D_CCE_DOUBLE_EX10 */
{
    /*
    ...
    */
    ...
    if(LEGEND_PRINT) {
        ...
        printf(" DFTI_BACKWARD_SCALE = 1.0/(double)(n[0]*n[1]) \n\n");
    }

    /*
    /   Allocate arrays for input and expected data
    */
    nn_in = (long)n[0] * (long)n[1];
    nn_out= ((long)n[0] * ((long)n[1]/2+1)) * 2;
    x_in  = (double*)malloc(nn_out*sizeof(double));
    x_exp = (double*)malloc(nn_out*sizeof(double));
    /*
    /   initialize x_in and copy to expected x_exp
    */
    zero_init_d(x_in, nn_out);
    init_real_vectors_d(x_in, nn_out);
    cblas_dcopy(nn_out, x_in, 1, x_exp, 1);

    /*
    /   Create Dfti descriptor for 2D double precision forward transform
    */
    Desc_Handle = fftw_plan_dft_r2c(rank, n, x_in, (fftw_complex*)x_in, FFTW_ESTIMATE);
    ...
    */
    Desc_Handle = fftw_plan_dft_c2r(rank, n, (fftw_complex*)x_in, x_in, FFTW_ESTIMATE);
    /*
    /   Compute DFT
    */
    fftw_execute(Desc_Handle);
    ...
    */
    ...
    /   Result scaling
    */
    scaling_2d_d(x_in, Scale, n[0], n[1]);

    /*
    /   Check result
    */
    maxerr = check_result_2d_d(x_in, x_exp, n[0], n[1]);
    if(ACCURACY_PRINT)
        printf("\n ACCURACY = %g\n\n", maxerr);
