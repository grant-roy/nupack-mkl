...
int main(int argc, char *argv[])  /* REAL_2D_CCS_DOUBLE_EX6 */
{
    double  x_in [M_MAX][N_MAX][K_MAX];
    double  x_out[M_MAX+2][N_MAX][K_MAX+2];
    ...
    */
    ...
    strides_out[1] = N_MAX*(K_MAX+2);
    strides_out[2] = 1;

    if(LEGEND_PRINT) {
        printf(" \n\n REAL_2D_CCS_DOUBLE_EX6                \n");
    ...
    */
    ...
    zero_init_d(x_out, (M_MAX+2)*N_MAX*(K_MAX+2));
    ...
    */
    ...
    if(ADVANCED_DATA_PRINT){
        ...
        for(i=0; i< multiple; i++) {
            printf("\n Transform Serial=%d \n", i);
            print_three_first_and_three_last_columns_d(&x_out[0][i][0], m+2, n+2, strides_out);
        }
    }
