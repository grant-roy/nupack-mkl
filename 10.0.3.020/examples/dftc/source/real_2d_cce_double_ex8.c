...
int main(int argc, char *argv[])  /* REAL_2D_CCE_DOUBLE_EX8 */
{
    double  x_in [M_MAX][N_MAX][K_MAX];
    mkl_double_complex  x_out[M_MAX][N_MAX/2+1][K_MAX];
    ...
    */
    ...
    strides_out[1] = (N_MAX/2+1)*K_MAX;
    strides_out[2] = K_MAX;

    if(LEGEND_PRINT) {
        printf(" \n\n REAL_2D_CCE_DOUBLE_EX8                \n");
        printf(" Forward-Backward 2D real transform for double precision data\n\n");
        printf(" Multiple 2D transform along the 3-d dimension of 3D array             \n");
    ...
    */
    ...
    if( first_index + multiple > K_MAX) {
        printf(" Error input parameters: first_index + multiple > K_MAX \n");
        printf(" Please see mkl_dfti_examples.h file\n");
        printf(" TEST FAIL\n");  goto END_OF_TEST;
    }
    ...
    */
    ...
    zero_init_z(x_out, M_MAX*(N_MAX/2+1)*K_MAX);

    init_real_mult_2d_d(x_in, m, n, multiple, dist_in, strides_in);

    cblas_dcopy(M_MAX*N_MAX*K_MAX, (const double*)x_in, 1, (double*)x_exp, 1);

     printf("\n 1 INPUT X, 3 first columns \n");
    if(ADVANCED_DATA_PRINT){
        printf("\n INPUT X, 3 first columns multiple=%d\n", multiple);
        for(i=0; i< multiple; i++) {
            printf("\n Transform Serial=%d \n", i);
            print_three_2d_columns_d(&x_in[0][0][i], m, n, strides_in);
        }
    }
    ...
    */
    ...
    if(ADVANCED_DATA_PRINT){
        printf("\n Forward result X, 3 2D columns \n\n");
        for(i=0; i< multiple; i++) {
            printf("\n Transform Serial=%d \n", i);
            print_three_2d_columns_z(&x_out[0][0][i], m, strides_out);
        }
    }
