...
int main(int argc, char *argv[])  /* REAL_2D_CCS_SINGLE_EX8 */
{
    float  x_in [M_MAX][N_MAX][K_MAX];
    float  x_out[M_MAX+2][N_MAX+2][K_MAX];
    ...
    */
    ...
    zero_init_s(x_out, (M_MAX+2)*(N_MAX+2)*K_MAX);
    ...
    */
    ...
    if(ADVANCED_DATA_PRINT){
        ...
        for(i=0; i< multiple; i++) {
            printf("\n Transform Serial=%d \n", i);
            print_three_first_and_three_last_columns_s(&x_out[0][0][i], m+2, n+2, strides_out);
        }
    }
