...
!
!       Perform multiple row one-dimensional transforms
...
!
!           DFTI_INPUT_STRIDES  = {first_index, 0}  (default={0,0})
...
int main(int argc, char *argv[])  /* COMPLEX_1D_DOUBLE_EX3 */
{
    ...
    */
    ...
    init_multiple_rows_z(x_in, n, multiple, first_index, dist_in);

    cblas_zcopy(M_MAX*N_MAX, x_in, 1, x_exp, 1);

    if(ADVANCED_DATA_PRINT){
        printf("\n INPUT X by columns (for multiple=3)\n\n");
        print_three_rows_z(x_in, n, first_index, dist_in);
    }
    ...
    */
    ...
    if(ADVANCED_DATA_PRINT){
        printf("\n Fortward result X by columns (for multiple=3)\n\n");
        print_three_rows_z(x_in, n, first_index, dist_in);
    }
    /*
