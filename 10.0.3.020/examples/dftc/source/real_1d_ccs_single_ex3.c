...
int main(int argc, char *argv[])  /* REAL_1D_CCS_SINGLE_EX3 */
{
    float   x_in [M_MAX*(N_MAX+2)];
    float   x_exp[M_MAX*(N_MAX+2)];
    ...
    */
    zero_init_s(x_in, M_MAX*(N_MAX+2));

    init_multiple_columns_s(x_in, n, multiple, dist_in, strides_in);

    cblas_scopy(M_MAX*(N_MAX+2), x_in, 1, x_exp, 1);
    ...
    */
    ...
    if(ADVANCED_DATA_PRINT){
        ...
        print_three_columns_s(x_in, n+2, dist_in, strides_in);
    }
    ...
    */
    ...
    /   Check result
    */
    maxerr = check_result_s(x_in, x_exp,M_MAX*(N_MAX+2));
    if(ACCURACY_PRINT)
        printf("\n ACCURACY = %g\n\n", maxerr);
