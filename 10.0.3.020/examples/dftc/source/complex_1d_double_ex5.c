...
!
!       Complex-to-complex multiple 1D transform for double precision data
!       which are allocated in two-dimension array, inplace.
!
!       Perform multiple column one-dimensional transforms
!       Note that column data are not unit-stride
!
!*******************************************************************************
...
!
!           DFTI_INPUT_STRIDES  = {first_index, step_x}     (default={0,1})
!           DFTI_NUMBER_OF_TRANSFORMS = multiple            (default = 1)
!           DFTI_INPUT_DISTANCE       = dist_x   (obligatory, if NUMBER_OF_TRANSFORMS >1)
!
...
int main(int argc, char *argv[])  /* COMPLEX_1D_DOUBLE_EX5 */
{
    ...
    */
    ...
    if(LEGEND_PRINT) {
        printf(" \n\n COMPLEX_1D_DOUBLE_EX5                                     \n");
        printf(" Forward-Backward multiple 1D complex transform                 \n");
        printf(" Note that column data are not unit-stride                  \n\n");
    ...
    */
    ...
    init_multiple_columns_z(x_in, n, multiple, first_index, N_MAX);

    cblas_zcopy(M_MAX*N_MAX, x_in, 1, x_exp, 1);

    if(ADVANCED_DATA_PRINT){
        printf("\n INPUT  X (for multiple=3)\n\n");
        print_three_columns_z(x_in, n, first_index, N_MAX);
    }
    ...
    */
    ...
    if(ADVANCED_DATA_PRINT){
        printf(" Forward result X by columns (for multiple=3)\n\n");
        print_three_columns_z(x_in, n, first_index, N_MAX);
    }
