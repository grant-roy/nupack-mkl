...
int main(int argc, char *argv[])  /* REAL_1D_CCS_DOUBLE_EX5 */
{
    double  x_in [(M_MAX+2)*N_MAX];
    double  x_exp[(M_MAX+2)*N_MAX];
    ...
    */
    zero_init_d(x_in, (M_MAX+2)*N_MAX);

    init_multiple_columns_d(x_in, n, multiple, dist_in, strides_in);

    cblas_dcopy((M_MAX+2)*N_MAX, x_in, 1, x_exp, 1);
