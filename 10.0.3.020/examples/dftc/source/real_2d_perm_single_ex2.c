...
int main(int argc, char *argv[])  /* REAL_2D_PERM_SINGLE_EX2 */
{
    ...
    */
    one_init_s(x_in, M_MAX*N_MAX);
    one_init_s(x_out,M_MAX*N_MAX);

    init_real_2d_s(x_in, m, n, strides_in);

    cblas_scopy(M_MAX*N_MAX, (const float*)x_in, 1, (float*)x_exp, 1);

    if(ADVANCED_DATA_PRINT){
        printf("\n INPUT X, 3 first columns and 3 last columns \n\n");
        print_three_first_and_three_last_columns_s(x_in, m, n, strides_in);
    }
    ...
    */
    ...
    if(ADVANCED_DATA_PRINT){
        ...
        print_three_first_and_three_last_columns_s(x_out, m, n, strides_out);
    }
