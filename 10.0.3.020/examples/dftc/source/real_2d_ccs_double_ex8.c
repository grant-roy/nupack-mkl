...
int main(int argc, char *argv[])  /* REAL_2D_CCS_DOUBLE_EX8 */
{
    double  x_in [M_MAX][N_MAX][K_MAX];
    double  x_out[M_MAX+2][N_MAX+2][K_MAX];
    ...
    */
    ...
    strides_out[1] = (N_MAX+2)*K_MAX;
    strides_out[2] = K_MAX;

    if(LEGEND_PRINT) {
        printf(" \n\n REAL_2D_CCS_DOUBLE_EX8                \n");
    ...
    */
    ...
    zero_init_d(x_out, (M_MAX+2)*(N_MAX+2)*K_MAX);
    ...
    */
    ...
    if(ADVANCED_DATA_PRINT){
        printf("\n Forward result X, 3 first and 3 last columns \n\n");
        for(i=0; i< multiple; i++) {
            printf("\n Transform Serial=%d \n", i);
            print_three_first_and_three_last_columns_d(&x_out[0][0][i], m+2, n+2, strides_out);
        }
    }
