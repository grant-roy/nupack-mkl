...
int main(int argc, char *argv[])  /* REAL_1D_CCE_DOUBLE_EX6 */
{
    double  x_in [M_MAX*N_MAX];
    mkl_double_complex  x_out[(M_MAX/2+1)*N_MAX];
    ...
    */
    ...
    zero_init_z(x_out, (M_MAX/2+1)*N_MAX);
    ...
    */
    ...
    if(ADVANCED_DATA_PRINT){
        ...
        print_three_columns_complex_z(x_out, n/2+1, dist_out, strides_out);
    }
