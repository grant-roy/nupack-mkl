...
int main(int argc, char *argv[])  /* REAL_2D_CCS_DOUBLE_EX1 */
{
    double  x_in [M_MAX+2][N_MAX+2];
    double  x_exp[M_MAX+2][N_MAX+2];
    ...
    */
    one_init_d(x_in, (M_MAX+2)*(N_MAX+2));

    init_real_2d_d(x_in, m, n, strides_in);

    cblas_dcopy((M_MAX+2)*(N_MAX+2), (const double*)x_in, 1, (double*)x_exp, 1);
