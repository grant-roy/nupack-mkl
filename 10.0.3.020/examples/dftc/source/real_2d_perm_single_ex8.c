...
int main(int argc, char *argv[])  /* REAL_2D_PERM_SINGLE_EX8 */
{
    float  x_in [M_MAX][N_MAX][K_MAX];
    float  x_out[M_MAX][N_MAX][K_MAX];
    ...
    */
    ...
    zero_init_s(x_out, M_MAX*N_MAX*K_MAX);
    ...
    */
    ...
    if(ADVANCED_DATA_PRINT){
        ...
        for(i=0; i< multiple; i++) {
            printf("\n Transform Serial=%d \n", i);
            print_three_2d_columns_s(&x_out[0][0][i], m, n, strides_out);
        }
    }
