...
int main(int argc, char *argv[])  /* REAL_2D_CCS_SINGLE_EX7 */
{
    float  x_in [M_MAX+2][N_MAX+2][K_MAX];
    float  x_exp[M_MAX+2][N_MAX+2][K_MAX];
    ...
    */
    zero_init_s(x_in, (M_MAX+2)*(N_MAX+2)*K_MAX);

    init_real_mult_2d_s(x_in, m, n, multiple, dist_in, strides_in);

    cblas_scopy((M_MAX+2)*(N_MAX+2)*K_MAX, (const float*)x_in, 1, (float*)x_exp, 1);

    if(ADVANCED_DATA_PRINT){
        printf("\n INPUT X, 3 first columns and 3 last columns \n\n");
        for(i=0; i< multiple; i++) {
            printf("\n Transform Serial=%d \n", i);
            print_three_first_and_three_last_columns_s(&x_in[0][0][i], m+2, n+2, strides_in);
        }
    }
