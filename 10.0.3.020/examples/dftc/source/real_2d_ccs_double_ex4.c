...
int main(int argc, char *argv[])  /* REAL_2D_CCS_DOUBLE_EX4 */
{
    double  x_in [M_MAX][N_MAX][K_MAX];
    double  x_out[M_MAX][N_MAX+2][K_MAX+2];
    ...
    */
    ...
    zero_init_d(x_out, M_MAX*(N_MAX+2)*(K_MAX+2));
    ...
    */
    ...
    if(ADVANCED_DATA_PRINT){
        ...
        for(i=0; i< multiple; i++) {
            printf("\n Transform Serial=%d \n", i);
            print_three_first_and_three_last_columns_d(&x_out[i][0][0], m+2, n+2, strides_out);
        }
    }
