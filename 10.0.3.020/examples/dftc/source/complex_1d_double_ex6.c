...
int main(int argc, char *argv[])  /* COMPLEX_1D_DOUBLE_EX6 */
{
    mkl_double_complex  x_in[M_MAX][N_MAX];
    mkl_double_complex  x_out[M_MAX][N_MAX];
    ...
    */
    ...
    if(ADVANCED_DATA_PRINT){
        printf(" Forward result X_OUT by columns (for multiple=3)\n\n");
        print_three_columns_z(x_out, n, first_index, N_MAX);
    }
    ...
    */
    ...
    if(ADVANCED_DATA_PRINT){
        printf(" Backward result X_IN by columns (for multiple=3)\n\n");
        print_three_columns_z(x_in, n, first_index, N_MAX);
    }
