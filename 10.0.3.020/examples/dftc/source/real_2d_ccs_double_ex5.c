...
int main(int argc, char *argv[])  /* REAL_2D_CCS_DOUBLE_EX5 */
{
    double  x_in [M_MAX+2][N_MAX][K_MAX+2];
    double  x_exp[M_MAX+2][N_MAX][K_MAX+2];
    ...
    */
    ...
    strides_in[1] = N_MAX * (K_MAX+2);
    strides_in[2] = 1;

    if(LEGEND_PRINT) {
        printf(" \n\n REAL_2D_CCS_DOUBLE_EX5                \n");
    ...
    */
    zero_init_d(x_in, (M_MAX+2)*N_MAX*(K_MAX+2));

    init_real_mult_2d_d(x_in, m, n, multiple, dist_in, strides_in);

    cblas_dcopy((M_MAX+2)*N_MAX*(K_MAX+2), (const double*)x_in, 1, (double*)x_exp, 1);

    if(ADVANCED_DATA_PRINT){
        printf("\n INPUT X, 3 first columns and 3 last columns \n\n");
        for(i=0; i< multiple; i++) {
            printf("\n Transform Serial=%d \n", i);
            print_three_first_and_three_last_columns_d(&x_in[0][i][0], m+2, n+2, strides_in);
        }
    }
