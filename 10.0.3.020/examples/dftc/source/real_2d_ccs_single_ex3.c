...
int main(int argc, char *argv[])  /* REAL_2D_CCS_SINGLE_EX3 */
{
    float  x_in [M_MAX][N_MAX+2][K_MAX+2];
    float  x_exp[M_MAX][N_MAX+2][K_MAX+2];
    ...
    */
    zero_init_s(x_in, M_MAX*(N_MAX+2)*(K_MAX+2));

    init_real_mult_2d_s(x_in, m, n, multiple, dist_in, strides_in);

    cblas_scopy(M_MAX*(N_MAX+2)*(K_MAX+2), (const float*)x_in, 1, (float*)x_exp, 1);

    if(ADVANCED_DATA_PRINT){
        printf("\n INPUT X, 3 first columns and 3 last columns \n\n");
        for(i=0; i< multiple; i++) {
            printf("\n Transform Serial=%d \n", i);
            print_three_first_and_three_last_columns_s(&x_in[i][0][0], m+2, n+2, strides_in);
        }
    }
