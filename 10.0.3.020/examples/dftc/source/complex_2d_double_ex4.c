...
!           DFTI_INPUT_STRIDES  = {first_index, stride_out_m, stride_out_n} (default={0, n, 1})
!
!           DFTI_NUMBER_OF_TRANSFORMS   = multiple                      (default = 1)
!           DFTI_INPUT_DISTANCE         = dist_in   (obligatory, if NUMBER_OF_TRANSFORMS >1)
!           DFTI_OUTPUT_DISTANCE        = dist_out   (obligatory, if NUMBER_OF_TRANSFORMS >1)
!
...
int main(int argc, char *argv[])  /* COMPLEX_2D_DOUBLE_EX4 */
{
    mkl_double_complex  x_in [M_MAX][N_MAX][K_MAX];
    mkl_double_complex  x_out[M_MAX][N_MAX][K_MAX];
    ...
    */
    ...
    if(LEGEND_PRINT) {
        printf(" COMPLEX_2D_DOUBLE_EX4  \n");
        printf(" Forward-Backward 2D complex transform for double precision data    \n\n");
        printf(" Multiple 2D transform along 1-st dimension of 3D array             \n");
        ...
        printf(" DFTI_OUTPUT_STRIDES        = {%d, %d, %d}\n", first_index,
                                                strides_out[1], strides_out[2]);
    ...
    /   for multiple 2D transform along 1-st dimension of 3D array
    */
    if((first_index + multiple*(m*n) > M_MAX*N_MAX*K_MAX ) ||
       (first_index + multiple*(m*n) > M_MAX*N_MAX*K_MAX )){
    ...
    */
    ...
    zero_init_z(x_out, M_MAX*N_MAX*K_MAX);

    for(i=0; i< multiple; i++){
        init_multiple_2d_columns_z(&x_in[i][0][0], m, n, strides_in);
    }
    ...
    if(ADVANCED_DATA_PRINT){
        ...
        for(i=0; i< multiple; i++) {
            printf("\n Transform Serial=%d \n", i);
            print_three_2d_columns_z(&x_in[i][0][0], m, strides_in);
        }
    }
    ...
    */
    ...
    if(ADVANCED_DATA_PRINT){
        ...
        for(i=0; i< multiple; i++) {
            printf("\n Transform Serial=%d \n", i);
            print_three_2d_columns_z(&x_out[i][0][0], m, strides_out);
        }
    }
