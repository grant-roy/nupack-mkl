...
int main()
{
  ...
  for(i=0;i<vec_len;i++) {
    fA1[i]=(float)(__SSUB_BEG+((__SSUB_END-__SSUB_BEG)*i)/vec_len);
    fA2[i]=(float)(__SSUB_END-((__SSUB_END-__SSUB_BEG)*i)/vec_len);
    fB1[i]=0.0;
    fB2[i]=(float)(fA1[i]-fA2[i]);
  }

  vsSub(vec_len,fA1,fA2,fB1);

  printf("vsSub test/example program\n\n");
  printf("           Argument                     vsSub                      -\n");
  printf("===============================================================================\n");
  for(i=0;i<vec_len;i++) {
