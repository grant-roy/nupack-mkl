...
int main()
{
  double dA1[VEC_LEN],dB1[VEC_LEN],dB2[VEC_LEN];

  MKL_INT i=0,vec_len=VEC_LEN;
  double CurRMS,MaxRMS=0.0;

  for(i=0;i<vec_len;i++) {
    dA1[i]=(double)(__DABS_BEG+((__DABS_END-__DABS_BEG)*i)/vec_len);
    dB1[i]=0.0;
    dB2[i]=fabs(dA1[i]);
  }

  vdAbs(vec_len,dA1,dB1);

  printf("vdAbs test/example program\n\n");
  printf("           Arguments                    vdAbs                    abs\n");
  printf("===============================================================================\n");
  for(i=0;i<vec_len;i++) {
    printf("% 13.6f             % 25.14e % 25.14e\n",dA1[i],dB1[i],dB2[i]);
