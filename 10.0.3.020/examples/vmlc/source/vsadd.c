...
int main()
{
  ...
  for(i=0;i<vec_len;i++) {
    fA1[i]=(float)(__SADD_BEG+((__SADD_END-__SADD_BEG)*i)/vec_len);
    fA2[i]=(float)(__SADD_BEG+((__SADD_END-__SADD_BEG)*i)/vec_len);
    fB1[i]=0.0;
    fB2[i]=(float)(fA1[i]+fA2[i]);
  }

  vsAdd(vec_len,fA1,fA2,fB1);

  printf("vsAdd test/example program\n\n");
  printf("           Argument                     vsAdd                      +\n");
  printf("===============================================================================\n");
  for(i=0;i<vec_len;i++) {
