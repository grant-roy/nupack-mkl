...
int main()
{
  ...
  for(i=0;i<vec_len;i++) {
    fA1[i]=(float)(__SMUL_BEG+((__SMUL_END-__SMUL_BEG)*i)/vec_len);
    fA2[i]=(float)(__SMUL_END-((__SMUL_END-__SMUL_BEG)*i)/vec_len);
    fB1[i]=0.0;
    fB2[i]=(float)(fA1[i]*fA2[i]);
  }

  vsMul(vec_len,fA1,fA2,fB1);

  printf("vsMul test/example program\n\n");
  printf("           Argument                     vsMul                      *\n");
  printf("===============================================================================\n");
  for(i=0;i<vec_len;i++) {
