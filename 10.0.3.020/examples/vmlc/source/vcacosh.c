...
int main()
{
  ...
  for(i=0;i<vec_len;i++) {
    cA[i].real=(float)(__SACOSH_BEG+((__SACOSH_END-__SACOSH_BEG)*i)/vec_len);
    cA[i].imag=(float)(__SACOSH_END-((__SACOSH_END-__SACOSH_BEG)*i)/vec_len);
    cB1[i].real=0.0;
    cB1[i].imag=0.0;
  }

  vcAcosh(vec_len,cA,cB1);
