...
int main(void)
{
	...
	/*---------------------------------------------------------------------------*/
	MKL_INT n=100, rci_request, itercount, i;
	double rhs[100];
	MKL_INT ia[100+1];
	MKL_INT ja[100-1];
	...
	double omega=0.5E0, one=1.E0, zero=0.E0, om=1.E0-0.5E0;
	...
	/*---------------------------------------------------------------------------*/
	/* Initialize the coeffient matrix and expected solution                     */
	/*---------------------------------------------------------------------------*/
	for(i=0;i<n;i++) expected_sol[i]=1.E0;
...
rci: dcg(&n,solution,rhs,&rci_request,ipar,dpar,tmp);
	...
	if (rci_request==2)
	{
		...
		/*---------------------------------------------------------------------------*/
		if (euclidean_norm>1.E-6) goto rci;
		/*---------------------------------------------------------------------------*/
...
getsln: dcg_get(&n,solution,rhs,&rci_request,ipar,dpar,tmp,&itercount);
	...
	for (i=0;i<n/4;i++)
	{
		printf("%6.3f  %6.3f  %6.3f  %6.3f",solution[4*i],solution[4*i+1],
			                                       solution[4*i+2],solution[4*i+3]);
		printf("\n");
	}
	printf("\nExpected solution is\n");
	for (i=0;i<n/4;i++)
	{
		printf("%6.3f  %6.3f  %6.3f  %6.3f",expected_sol[4*i],expected_sol[4*i+1],
			                               expected_sol[4*i+2],expected_sol[4*i+3]);
		printf("\n");
	}

	printf("\nNumber of iterations: %d\n",itercount);
	goto end;
failure: printf("RCI CG solver failed to complete computations. Error code %d", rci_request);
end: return 0;
}
