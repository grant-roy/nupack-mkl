...
int main(void)
{
	...
	/*---------------------------------------------------------------------------*/
	MKL_INT n=8, rci_request, itercount, i;
	double rhs[8];
...
rci:  dcg(&n,solution,rhs,&rci_request,ipar,dpar,tmp);
	...
	if (rci_request==2)
	{
		...
		/*---------------------------------------------------------------------------*/
		if (euclidean_norm>1.e-8) goto rci;
		/*---------------------------------------------------------------------------*/
...
getsln: dcg_get(&n,solution,rhs,&rci_request,ipar,dpar,tmp,&itercount);
	...
	for (i=0;i<n/2;i++) printf("%6.3f  ",expected_sol[i]);
	printf("\n");
	for (i=n/2;i<n;i++) printf("%6.3f  ",expected_sol[i]);
