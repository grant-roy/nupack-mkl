...
real bd_ax(ny+1,nz+1), bd_bx(ny+1,nz+1), bd_ay(nx+1,nz+1), bd_by(nx+1,nz+1), bd_az(nx+1,ny+1), bd_bz(nx+1,ny+1)
real q
...
! Filling in the right-hand side f(x,y,z)=(12*pi*pi)*sin(2*pi*x)*sin(2*pi*y)*sin(2*pi*z)
...
! Initializing simple data structures of Poisson Library for 3D Poisson Solver
   call s_init_Helmholtz_3D(ax, bx, ay, by, az, bz, nx, ny, nz, BCtype, q, ipar, spar, stat)
...
! you should save it in another memory location!
   call s_commit_Helmholtz_3D(f, bd_ax, bd_bx, bd_ay, bd_by, bd_az, bd_bz, xhandle, yhandle, ipar, spar, stat)
   if (stat.ne.0) goto 999
...
! Otherwise the results may be wrong.
   call s_Helmholtz_3D(f, bd_ax, bd_bx, bd_ay, bd_by, bd_az, bd_bz, xhandle, yhandle, ipar, spar, stat)
   if (stat.ne.0) goto 999
...
! Success message to print if everything is OK
	print *, ' real 3D Poisson example has successfully PASSED'
	print *, ' through all steps of computation!'
! Jumping over failure message
    go to 1
! Failure message to print if something went wrong
999 print *, 'real 3D Poisson example FAILED to compute the solution...'
...
12    format(1x,'In the mesh point (',F4.2,',',F4.2,',',F4.2,') the error between the computed and the true solution is equal to ', E10.3)
