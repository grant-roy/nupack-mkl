...
do it=1,nt+1
   do ip=1,np+1
      ...
      u(ip,it)=ct*cp
      f(ip,it)=q*ct*cp+(2.0D0-(dcos(theta_i)/dsin(theta_i))*(at+bt-2*theta_i))*cp+2.0D0*ct/(dsin(theta_i)*dsin(theta_i))
   enddo
enddo
...
! Success message to print if everything is OK
	print *, ' Double precision Helmholtz example in a spherical rectangle has successfully'
...
999 print *, 'Double precision Helmholtz example in a spherical rectangle FAILED to compute the solution...'
