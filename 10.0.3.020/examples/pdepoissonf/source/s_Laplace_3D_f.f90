...
do iz=1,nz+1
   do iy=1,ny+1
      do ix=1,nx+1
         ...
         u(ix,iy,iz)=xi**4+yi**4+zi**4-3.0E0*((xi**2)*(yi**2)+(xi**2)*(zi**2)+(yi**2)*(zi**2))
         f(ix,iy,iz)=0.0E0
      enddo
   enddo
enddo
...
! Success message to print if everything is OK
	print *, ' real 3D Laplace example has successfully PASSED'
	print *, ' through all steps of computation!'
! Jumping over failure message
    go to 1
! Failure message to print if something went wrong
999 print *, 'real 3D Laplace example FAILED to compute the solution...'
