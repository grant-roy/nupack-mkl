...
!
!*******************************************************************************
!   function    : rfftwnd_create_plan - wrapper for interface FFTW function
...
rfftwnd_plan rfftwnd_create_plan(int rank, const int* n, fftw_direction dir,
	...
	if (rank > 1) {
		...
		if (rank == 3) {
	        ...
	        strides_out[0] = 0;
	        strides_out[1] = 2 * (nn[1]/2+1);
	        strides_out[2] = 1;
		}

        Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_PRECISION, DFTI_REAL, rank, nn);
		...
		Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_PRECISION, DFTI_REAL, 1, nn[0]);
        if (! DftiErrorClass(Status, DFTI_NO_ERROR)) {
            free(fftw_desc_mkl);
            free(nn);
            return NULL;
