...
!
!*******************************************************************************
!   function    : fftw_plan_dft_r2c_3d - wrapper for interface FFTW function
!   Content     :
!   output data :
!*******************************************************************************
*/
...
fftw_plan fftw_plan_dft_r2c_3d(int nx, int ny, int nz, double *in,
	...
	strides_in[0] = 0;
	strides_in[1] = 2 * (nn[2]/2+1) * nn[1];
	strides_in[2] = 2 * (nn[2]/2+1);
	strides_in[3] = 1;
    ...
    Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_DOUBLE, DFTI_REAL, 3, nn);
