...
!
!*******************************************************************************
!   function    : sfftw_plan_many_dft_c2r - interface FFTW initialization function
!   Content     : wrapper from this function to DFTI MKL initialization functions
!                 for single precision complex data
!   input data  : parameters for REAL DFT
...
void sfftw_plan_many_dft_c2r(fftwf_plan* plan, int* rank, const int* n, int* howmany,
...
int i;
    ...
    *plan = fftwf_plan_many_dft_c2r(*rank, nn, *howmany, in, inem, *istride, *idist,
...
/*    long Status, i;
    ...
    if ((float*)out == in) return NULL; /* in-place DFT is not implemented */
...
/*    fftw_desc_mkl = (fftw_plan_mkl*)malloc(sizeof(fftw_plan_mkl));
	...
	if (rank == 1) {
        Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_SINGLE, DFTI_REAL, (long)rank, nn[0]);
	} else {
        Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_SINGLE, DFTI_REAL, (long)rank, nn);
    }
