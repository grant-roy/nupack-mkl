...
!
!*******************************************************************************
!   function    : dfftw_plan_many_dft - interface FFTW initialization function
...
void dfftw_plan_many_dft(fftw_plan* plan, int* rank, const int* n, int* howmany,
...
/*	long Status, i;
	...
	if (rank == 1) {
        Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_DOUBLE, DFTI_COMPLEX, (long)rank, nn[0]);
	} else {
        Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_DOUBLE, DFTI_COMPLEX, (long)rank, nn);
    }
