...
!
!*******************************************************************************
!   function    : dfftw_plan_many_dft_c2r - interface FFTW initialization function
...
void dfftw_plan_many_dft_c2r(fftw_plan* plan, int* rank, const int* n, int* howmany,
...
int i;
    ...
    *plan = fftw_plan_many_dft_c2r(*rank, nn, *howmany, in, inem, *istride, *idist,
...
/*    fftw_desc_mkl = (fftw_plan_mkl*)malloc(sizeof(fftw_plan_mkl));
	...
	if (rank == 1) {
        Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_DOUBLE, DFTI_REAL, (long)rank, nn[0]);
	} else {
        Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_DOUBLE, DFTI_REAL, (long)rank, nn);
    }
