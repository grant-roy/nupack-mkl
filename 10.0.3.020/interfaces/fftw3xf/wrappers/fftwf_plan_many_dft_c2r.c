...
!
!*******************************************************************************
!   function  : fftwf_plan_many_dft_c2r - interface FFTW initialization function
...
fftwf_plan
fftwf_plan_many_dft_c2r(int rank, const int* n, int howmany,
                        ...
                        unsigned flags)
{
    ...
    if (rank == 1) {
        Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_SINGLE, DFTI_REAL, (MKL_LONG)rank, nn[0]);
    } else {
        Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_SINGLE, DFTI_REAL, (MKL_LONG)rank, nn);
    }
    ...
    for (i = rank-1; i>0; i--) {
        if (inembed != NULL) {
            ...
            if (i == rank-1) {
                strides[i] = (nn[i]/2+1) * strides[i+1];
            } else {
    ...
    for (i = rank-1; i>0; i--) {
        if (onembed != NULL) {
            ...
            if (i == rank-1 && out == (float*)in) {
                strides[i] = (nn[i]/2+1) * 2 * strides[i+1];
            } else {
                strides[i] = nn[i] * strides[i+1];
            }
        }
