...
void fftwnd_mpi_local_sizes(fftwnd_mpi_plan p,int *n,int *start,int *n_out,int *start_out, int *size){
	...
	*size=v;
	return;
error:
	fprintf(stderr,"CDFT error %ld in fftwnd_mpi_local_sizes wrapper\n",err);
	return;
}
