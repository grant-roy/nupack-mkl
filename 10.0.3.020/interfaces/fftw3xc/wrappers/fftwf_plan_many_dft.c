...
!
!*******************************************************************************
!   function    : fftwf_plan_many_dft - interface FFTW initialization function
...
fftwf_plan fftwf_plan_many_dft(int rank, const int* n, int howmany,
	...
	if (rank == 1) {
        Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_SINGLE, DFTI_COMPLEX, (MKL_LONG)rank, nn[0]);
	} else {
        Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_SINGLE, DFTI_COMPLEX, (MKL_LONG)rank, nn);
    }
