...
!
!*******************************************************************************
!   function    : fftw_plan_dft_r2c - wrapper for interface FFTW function
...
fftw_plan fftw_plan_dft_r2c(int rank, const int* n, double *in, fftw_complex *out, unsigned flags) {
	...
	if (rank > 1) {
		...
		if (rank == 3) {
			strides_in[0] = 0;
	        strides_in[1] = (nn[2]/2+1) * 2 * nn[1];
	        strides_in[2] = (nn[2]/2+1) * 2;
	        strides_in[3] = 1;

			strides_out[0] = 0;
	        strides_out[1] = (nn[2]/2+1) * nn[1];
	        strides_out[2] = nn[2]/2+1;
	        strides_out[3] = 1;
		} else {
            strides_in[0] = 0;
        ...
        Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_DOUBLE, DFTI_REAL, rank, nn);
        ...
        Status = DftiCreateDescriptor(&(fftw_desc_mkl->mkl_desc), DFTI_DOUBLE, DFTI_REAL, 1, nn[0]);
        if (! DftiErrorClass(Status, DFTI_NO_ERROR)) {
            free(fftw_desc_mkl);
            return NULL;
        }
