...
!
!*******************************************************************************
!   function    : fftwf_execute_dft_c2r - interface FFTW initialization function
...
fftwf_execute_dft_c2r(const fftwf_plan plan, fftwf_complex* in, float* out)
{
    ...
    if ((float*)in == out) {
        DftiComputeBackward( fftw_desc_mkl->mkl_desc, in);
    } else {
        DftiComputeBackward( fftw_desc_mkl->mkl_desc, in, out);
    }
    return;
}
