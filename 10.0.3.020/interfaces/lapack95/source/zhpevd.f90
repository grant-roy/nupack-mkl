...
PURE SUBROUTINE ZHPEVD_MKL95(A,W,UPLO,Z,INFO)
    ! MKL Fortran77 call:
    ! ZHPEVD(JOB,UPLO,N,A,W,Z,LDZ,WORK,LWORK,RWORK,LRWORK,IWORK,LIWORK,
    !   INFO)
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_HPEVD, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    LRWORK = -1
    LWORK = -1
    CALL MKL77_HPEVD(JOB,O_UPLO,N,A,W,O_Z,LDZ,S_WORK,LWORK,S_RWORK,     &
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_HPEVD(JOB,O_UPLO,N,A,W,O_Z,LDZ,WORK,LWORK,RWORK,     &
