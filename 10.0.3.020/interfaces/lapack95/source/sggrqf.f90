...
PURE SUBROUTINE SGGRQF_MKL95(A,B,TAUA,TAUB,INFO)
    ...
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_GGRQF, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    LWORK = -1
    CALL MKL77_GGRQF(M,P,N,A,LDA,O_TAUA,B,LDB,O_TAUB,S_WORK,LWORK,      &
     &                                                           O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_GGRQF(M,P,N,A,LDA,O_TAUA,B,LDB,O_TAUB,WORK,LWORK,    &
     &                                                           O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
