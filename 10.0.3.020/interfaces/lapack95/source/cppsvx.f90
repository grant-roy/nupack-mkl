...
PURE SUBROUTINE CPPSVX_MKL95(A,B,X,UPLO,AF,FACT,EQUED,S,FERR,BERR,RCOND,&
     &                                                             INFO)
    ! MKL Fortran77 call:
    ! CPPSVX(FACT,UPLO,N,NRHS,A,AF,EQUED,S,B,LDB,X,LDX,RCOND,FERR,BERR,
    !   WORK,RWORK,INFO)
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_PPSVX, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    IF(PRESENT(AF)) THEN
        O_AF => AF
    ELSE
        ALLOCATE(O_AF(N*(N+1)/2), STAT=L_STAT_ALLOC)
    ENDIF
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_PPSVX(O_FACT,O_UPLO,N,NRHS,A,O_AF,O_EQUED,O_S,B,LDB, &
