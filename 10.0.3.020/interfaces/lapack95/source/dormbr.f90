...
PURE SUBROUTINE DORMBR_MKL95(A,TAU,C,VECT,SIDE,TRANS,INFO)
    ...
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_ORMBR, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    LWORK = -1
    CALL MKL77_ORMBR(O_VECT,O_SIDE,O_TRANS,M,N,K,A,LDA,TAU,C,LDC,S_WORK,&
     &                                                     LWORK,O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_ORMBR(O_VECT,O_SIDE,O_TRANS,M,N,K,A,LDA,TAU,C,LDC,   &
     &                                                WORK,LWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
