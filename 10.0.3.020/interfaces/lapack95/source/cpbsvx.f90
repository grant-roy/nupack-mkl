...
PURE SUBROUTINE CPBSVX_MKL95(A,B,X,UPLO,AF,FACT,EQUED,S,FERR,BERR,RCOND,&
     &                                                             INFO)
    ! MKL Fortran77 call:
    ! CPBSVX(FACT,UPLO,N,KD,NRHS,A,LDA,AF,LDAF,EQUED,S,B,LDB,X,LDX,
    !   RCOND,FERR,BERR,WORK,IWORK,INFO)
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_PBSVX, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_PBSVX(O_FACT,O_UPLO,N,KD,NRHS,A,LDA,O_AF,LDAF,       &
