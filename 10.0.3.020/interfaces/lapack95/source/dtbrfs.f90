...
PURE SUBROUTINE DTBRFS_MKL95(A,B,X,UPLO,TRANS,DIAG,FERR,BERR,INFO)
    ! MKL Fortran77 call:
    ! DTBRFS(UPLO,TRANS,DIAG,N,KD,NRHS,A,LDA,B,LDB,X,LDX,FERR,BERR,WORK,
    !   IWORK,INFO)
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_TBRFS, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_TBRFS(O_UPLO,O_TRANS,O_DIAG,N,KD,NRHS,A,LDA,B,LDB,X, &
