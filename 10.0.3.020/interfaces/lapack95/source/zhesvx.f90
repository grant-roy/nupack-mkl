...
PURE SUBROUTINE ZHESVX_MKL95(A,B,X,UPLO,AF,IPIV,FACT,FERR,BERR,RCOND,   &
     &                                                             INFO)
    ! MKL Fortran77 call:
    ! ZHESVX(FACT,UPLO,N,NRHS,A,LDA,AF,LDAF,IPIV,B,LDB,X,LDX,RCOND,FERR,
    !   BERR,WORK,LWORK,RWORK,INFO)
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_HESVX, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    LWORK = -1
    CALL MKL77_HESVX(O_FACT,O_UPLO,N,NRHS,A,LDA,O_AF,LDAF,O_IPIV,B,LDB, &
     &            X,LDX,O_RCOND,O_FERR,O_BERR,S_WORK,LWORK,RWORK,O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_HESVX(O_FACT,O_UPLO,N,NRHS,A,LDA,O_AF,LDAF,O_IPIV,B, &
