...
PURE SUBROUTINE SPBRFS_MKL95(A,AF,B,X,UPLO,FERR,BERR,INFO)
    ! MKL Fortran77 call:
    ! SPBRFS(UPLO,N,KD,NRHS,A,LDA,AF,LDAF,B,LDB,X,LDX,FERR,BERR,WORK,
    !   IWORK,INFO)
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_PBRFS, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_PBRFS(O_UPLO,N,KD,NRHS,A,LDA,AF,LDAF,B,LDB,X,LDX,    &
