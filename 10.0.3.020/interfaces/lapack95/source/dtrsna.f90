...
PURE SUBROUTINE DTRSNA_MKL95(T,S,SEP,VL,VR,SELECT,M,INFO)
    ...
    !   LDWORK,IWORK,INFO)
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_TRSNA, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_TRSNA(JOB,HOWMNY,O_SELECT,N,T,LDT,O_VL,LDVL,O_VR,    &
     &                   LDVR,O_S,O_SEP,MM,O_M,WORK,LDWORK,IWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
