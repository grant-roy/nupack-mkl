...
PURE SUBROUTINE SSBGVX_MKL95(A,B,W,UPLO,Z,VL,VU,IL,IU,M,IFAIL,Q,ABSTOL, &
     &                                                             INFO)
    ! MKL Fortran77 call:
    ! SSBGVX(JOBZ,RANGE,UPLO,N,KA,KB,A,LDA,B,LDB,Q,LDQ,VL,VU,IL,IU,
    !   ABSTOL,M,W,Z,LDZ,WORK,IWORK,IFAIL,INFO)
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_SBGVX, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_SBGVX(JOBZ,RANGE,O_UPLO,N,KA,KB,A,LDA,B,LDB,O_Q,LDQ, &
     &   O_VL,O_VU,O_IL,O_IU,O_ABSTOL,O_M,W,O_Z,LDZ,WORK,IWORK,O_IFAIL, &
     &                                                           O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
