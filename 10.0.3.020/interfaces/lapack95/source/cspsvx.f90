...
PURE SUBROUTINE CSPSVX_MKL95(A,B,X,UPLO,AF,IPIV,FACT,FERR,BERR,RCOND,   &
     &                                                             INFO)
    ! MKL Fortran77 call:
    ! CSPSVX(FACT,UPLO,N,NRHS,A,AF,IPIV,B,LDB,X,LDX,RCOND,FERR,BERR,
    !   WORK,RWORK,INFO)
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_SPSVX, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_SPSVX(O_FACT,O_UPLO,N,NRHS,A,O_AF,O_IPIV,B,LDB,X,LDX,&
