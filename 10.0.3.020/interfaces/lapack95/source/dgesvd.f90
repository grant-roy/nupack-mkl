...
PURE SUBROUTINE DGESVD_MKL95(A,S,U,VT,WW,JOB,INFO)
    ...
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_GESVD, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    LWORK = -1
    CALL MKL77_GESVD(JOBU,JOBVT,M,N,A,LDA,S,O_U,LDU,O_VT,LDVT,S_WORK,   &
     &                                                     LWORK,O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_GESVD(JOBU,JOBVT,M,N,A,LDA,S,O_U,LDU,O_VT,LDVT,WORK, &
     &                                                     LWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
