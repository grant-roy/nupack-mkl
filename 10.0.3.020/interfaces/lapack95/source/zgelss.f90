...
PURE SUBROUTINE ZGELSS_MKL95(A,B,RANK,S,RCOND,INFO)
    ...
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_GELSS, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    LWORK = -1
    CALL MKL77_GELSS(M,N,NRHS,A,LDA,B,LDB,O_S,O_RCOND,O_RANK,S_WORK,    &
     &                                               LWORK,RWORK,O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_GELSS(M,N,NRHS,A,LDA,B,LDB,O_S,O_RCOND,O_RANK,WORK,  &
     &                                               LWORK,RWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
