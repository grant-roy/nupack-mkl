...
PURE SUBROUTINE ZTGSEN_MKL95(A,B,SELECT,ALPHA,BETA,IJOB,Q,Z,PL,PR,DIF,M,&
     &                                                             INFO)
    ! MKL Fortran77 call:
    ...
    LIWORK = -1
    LWORK = -1
    CALL MKL77_TGSEN(O_IJOB,WANTQ,WANTZ,SELECT,N,A,LDA,B,LDB,O_ALPHA,   &
    ...
    IF(L_STAT_ALLOC==0) THEN
     ...
     &   O_ALPHA,O_BETA,O_Q,LDQ,O_Z,LDZ,O_M,O_PL,O_PR,O_DIF,WORK,LWORK, &
     &                                              IWORK,LIWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
