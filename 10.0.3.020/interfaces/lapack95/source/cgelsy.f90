...
PURE SUBROUTINE CGELSY_MKL95(A,B,RANK,JPVT,RCOND,INFO)
    ...
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_GELSY, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    LWORK = -1
    CALL MKL77_GELSY(M,N,NRHS,A,LDA,B,LDB,O_JPVT,O_RCOND,O_RANK,S_WORK, &
     &                                               LWORK,RWORK,O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_GELSY(M,N,NRHS,A,LDA,B,LDB,O_JPVT,O_RCOND,O_RANK,    &
     &                                          WORK,LWORK,RWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
