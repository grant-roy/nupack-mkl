...
PURE SUBROUTINE SSPGVX_MKL95(A,B,W,ITYPE,UPLO,Z,VL,VU,IL,IU,M,IFAIL,    &
     &                                                      ABSTOL,INFO)
    ! MKL Fortran77 call:
    ! SSPGVX(ITYPE,JOBZ,RANGE,UPLO,N,A,B,VL,VU,IL,IU,ABSTOL,M,W,Z,LDZ,
    !   WORK,IWORK,IFAIL,INFO)
    ! <<< Use statements >>>
    USE MKL77_LAPACK, ONLY: MKL77_SPGVX, MKL77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL MKL77_SPGVX(O_ITYPE,JOBZ,RANGE,O_UPLO,N,A,B,O_VL,O_VU,O_IL,&
